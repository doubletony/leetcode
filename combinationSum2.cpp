...
public:
    ...
    bool combinationSumRec(vector<int> &candidates, int target, int start, int end, vector<vector<int> > & result )
    {
        ...
        int newEnd = quickSearch(candidates, newTarget, start+1, end);//get the proper end;
        if ( newEnd >= start)
        {
            int currenRsize = result.size();
            if (combinationSumRec(candidates, newTarget, start+1, newEnd, result) )
            {
    ...
    vector<vector<int> > removeDuplicated( vector<vector<int> >  &orinial)
    {

        set< vector<int> > s;
        for (int i = 0; i < orinial.size(); i++)
        {
            s.insert(orinial[i]);
        }
